...
import { ACPFileSystemHandlers, JsonRpcError } from '../src/core/acp-method-handlers';
...
describe('FileOperationsHandler', () => {
  ...
  describe('ACP method handlers', () => {
    ...
    it('should return proper JSON-RPC errors for invalid params', async () => {
      await expect(acpHandlers.handleFsReadTextFile({} as any))
        .rejects.toThrow(JsonRpcError);
      
      await expect(acpHandlers.handleFsWriteTextFile({} as any))
        .rejects.toThrow(JsonRpcError);
    });
