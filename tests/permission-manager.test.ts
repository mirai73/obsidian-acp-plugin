...
import { PermissionManagerImpl, PermissionConfig } from '../src/core/permission-manager';
import { ACPSessionHandlers, JsonRpcError } from '../src/core/acp-method-handlers';
import { SessionRequestPermissionParams, SessionRequestPermissionResult } from '../src/types/acp';

describe('PermissionManager', () => {
  let permissionManager: PermissionManagerImpl;
  let acpSessionHandlers: ACPSessionHandlers;
  let mockUserConfirmation: jest.Mock<Promise<SessionRequestPermissionResult>, [SessionRequestPermissionParams]>;
  ...
  describe('logOperation', () => {
    ...
    it('should not log operations when disabled via config', async () => {
      ...
      const noLogManager = new PermissionManagerImpl(config, mockUserConfirmation);
      mockUserConfirmation.mockResolvedValue({
        outcome: { outcome: 'selected', optionId: 'allow_once' }
      });
  ...
  describe('configuration updates', () => {
    it('should update user confirmation handler', async () => {
      ...
      const result = await permissionManager.requestPermission(createMockParams());
  ...
  describe('ACP session/request_permission handler', () => {
    it('should handle valid permission requests and delegate', async () => {
      ...
      const params = createMockParams();
      const result = await acpSessionHandlers.handleSessionRequestPermission(params);
