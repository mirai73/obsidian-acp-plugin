...
describe('SessionManager', () => {
  ...
  describe('createSession', () => {
    ...
    it('should handle agent communication failure gracefully', async () => {
      ...
      await expect(
        sessionManager.createSession('test-agent', mockJsonRpcClient, cwd)
      ).rejects.toThrow();
    });
    ...
    it('should enforce maximum session limit', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        await sessionManager.createSession('test-agent', mockJsonRpcClient);
      }

      // Attempt to create one more should fail
      await expect(
        sessionManager.createSession('test-agent', mockJsonRpcClient)
