...
describe('ACPClientImpl Integration', () => {
  ...
  describe('Method handler registration', () => {
    it('should register and call file operation handlers', async () => {
      const readHandler = jest.fn().mockResolvedValue({ content: 'test content' });
  ...
  describe('Error handling', () => {
    ...
    it('should throw error when sending requests without agents', async () => {
      await expect(acpClient.sendRequest('test/method', {}))
        .rejects.toThrow('No agents connected');
      
      expect(() => acpClient.sendNotification('test/notification', {}))
        .toThrow('No agents connected');
    });
  });
});