...
export class JsonRpcClient {
  ...
  constructor(options: JsonRpcClientOptions = {}) {
    ...
    logger.debug(LogCategory.PROTOCOL, 'JSON-RPC client initialized', {
  ...
  async sendRequest(
    ...
    if (!this.transport) {
      const error = errorHandler.createConnectionFailedError(
    ...
    if (
      ...
      (this.options.maxPendingRequests || 100)
    ) {
      const error = errorHandler.createJsonRpcError(
        ...
        'Too many pending requests',
        {
          ...
          limit: this.options.maxPendingRequests,
        }
      );
    ...
    return new Promise<any>((resolve, reject) => {
      ...
      this.requestTracker.trackRequest(
        ...
        (error: JsonRpcError) => {
          logger.error(
            ...
            'Request failed with JSON-RPC error',
            {
              method,
              id: request.id,
      ...
      try {
        ...
        logger.debug(LogCategory.PROTOCOL, 'Sending request message', {
          method,
          id: request.id,
          message: message,
          sessionId: this.sessionId,
   ...
   */
  close(): void {
    logger.debug(LogCategory.PROTOCOL, 'Closing JSON-RPC client', {
      sessionId: this.sessionId,
    });
    ...
    logger.debug(LogCategory.PROTOCOL, 'JSON-RPC client closed', {
      sessionId: this.sessionId,
    });
  }
      ...
      try {
        ...
        if ('code' in parsed && 'message' in parsed) {
          ...
          const errorInfo = errorHandler.handleError(
            ...
            ErrorCategory.PROTOCOL,
            {
              rawMessage: messageData.substring(0, 200),
              sessionId: this.sessionId,
            }
          );
        ...
        logger.debug(LogCategory.PROTOCOL, 'Processing incoming message', {
          messageType: isResponseMessage(message)
            ? 'response'
        ...
        if (isResponseMessage(message)) {
          ...
          if (message.id !== null && message.id !== undefined) {
            this.requestTracker.handleResponse(
              message.id,
              message.result,
              message.error
            );
        ...
        logger.error(
          LogCategory.PROTOCOL,
          'Error handling JSON-RPC message',
          {
            error: (error as Error).message,
            sessionId: this.sessionId,
          },
  ...
  private handleTransportClose(): void {
    ...
    logger.info(
      LogCategory.CONNECTION,
      'Transport closed, cancelled pending requests',
      {
        sessionId: this.sessionId,
      }
    );
   ...
   */
  cleanupOldRequests(maxAge: number = 300000): number {
    // 5 minutes default
