...
import {
  FileOperationsHandlerImpl,
  FileOperationsConfig,
} from './file-operations-handler';
...
export class ACPClientImpl extends EventEmitter implements ACPClient {
  ...
  private fsReadTextFileHandler?: (
    params: FsReadTextFileParams
  ) => Promise<FsReadTextFileResult>;
  private fsWriteTextFileHandler?: (
    params: FsWriteTextFileParams
  ) => Promise<void>;
  private sessionRequestPermissionHandler?: (
  ...
  private initializeFileOperations(config: FileOperationsConfig): void {
    ...
    this.acpFileSystemHandlers = new ACPFileSystemHandlers(
  ...
  async startAgentWithConfig(config: AgentConfig): Promise<void> {
    ...
    try {
      ...
      const jsonRpcClient = new JsonRpcClient({
        ...
        enableConnectionRecovery: this.options.enableConnectionRecovery,
        recoveryConfig: this.options.recoveryConfig,
      });
      ...
      try {
        ...
        throw new Error(
          `Failed to initialize agent connection: ${error.message}`
        );
      }
      ...
      this.emit(
        'agent-connected',
        config.id,
        this.getConnectionStatus(config.id)
      );
    } catch (error) {
      // Clean up on failure
      await this.processManager.stopAgent(config.id).catch(() => {});
  ...
  async initializeConnection(agentId?: string): Promise<InitializeResult> {
    const connection = agentId
      ...
      : this.getFirstAvailableConnection();
    ...
    try {
      const result = await connection.jsonRpcClient.sendRequest(
        'initialize',
        params
      );
   ...
   */
  async handleSessionRequestPermission(
    params: SessionRequestPermissionParams
  ): Promise<SessionRequestPermissionResult> {
   ...
   * Register method handlers
   */
  setFsReadTextFileHandler(
    handler: (params: FsReadTextFileParams) => Promise<FsReadTextFileResult>
  ): void {
    this.fsReadTextFileHandler = handler;
  }

  setFsWriteTextFileHandler(
    handler: (params: FsWriteTextFileParams) => Promise<void>
  ): void {
    this.fsWriteTextFileHandler = handler;
  }

  setSessionRequestPermissionHandler(
    handler: (
      params: SessionRequestPermissionParams
    ) => Promise<SessionRequestPermissionResult>
  ): void {
  ...
  setSessionUpdateHandler(
    handler: (params: SessionUpdateParams) => void
  ): void {
  ...
  getConnectionStatus(agentId: string): ConnectionStatus {
    ...
    return {
      ...
      capabilities,
      error:
        connection.processHealth.status === 'error'
          ? 'Process error'
          : undefined,
    };
  }
