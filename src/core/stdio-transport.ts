...
export class StdioTransport extends EventEmitter implements MessageTransport {
  ...
  send(message: string): void {
    ...
    try {
      ...
      // Write to stdin
      logger.debug(LogCategory.PROTOCOL, 'StdioTransport: Sending message', {
  ...
  private processBufferedMessages(): void {
    ...
    while ((delimiterIndex = this.messageBuffer.indexOf(delimiter)) !== -1) {
      ...
      try {
        ...
        logger.debug(LogCategory.PROTOCOL, 'StdioTransport: Received message', {
          agentId: this.agentId,
          messageLength: Buffer.byteLength(message, this.options.encoding),
          message: message,
        });
