...
export class StdioTransport extends EventEmitter implements MessageTransport {
  ...
  constructor(process: ChildProcess, agentId: string, options: StdioTransportOptions = {}) {
    super();
  ...
  send(message: string): void {
    if (!this.connectionState.connected || this.isClosing) {
      throw new Error(`Cannot send message: transport is not connected (agent: ${this.agentId})`);
    }
    
    if (!this.process.stdin || this.process.stdin.destroyed) {
      throw new Error(`Cannot send message: stdin is not available (agent: ${this.agentId})`);
    }
    
    try {
      // Add delimiter and encode message
      const messageWithDelimiter = message + this.options.messageDelimiter;
      const messageBuffer = Buffer.from(messageWithDelimiter, this.options.encoding);
      
      // Check message size
      if (messageBuffer.length > this.options.maxMessageSize) {
        throw new Error(`Message too large: ${messageBuffer.length} bytes exceeds limit of ${this.options.maxMessageSize} bytes`);
      }
      ...
      this.handleError(new Error(`Failed to send message to agent ${this.agentId}: ${error}`));
      throw error;
    }
  }
  ...
  close(): void {
    ...
    try {
      ...
      console.error(`Error closing transport for agent ${this.agentId}:`, error);
    }
  }
  ...
  private initializeConnection(): void {
    ...
    const timeout = setTimeout(() => {
      if (!this.connectionState.connected) {
        this.handleError(new Error(`Connection timeout for agent ${this.agentId}`));
      }
    ...
    if (this.process.stdout && this.process.stdin) {
      ...
      clearTimeout(timeout);
      this.handleError(new Error(`Failed to initialize stdio streams for agent ${this.agentId}`));
    }
  }
  ...
  private setupProcessHandlers(): void {
    ...
    if (this.process.stdout) {
      ...
      this.process.stdout.on('error', (error) => {
        this.handleError(new Error(`Stdout error for agent ${this.agentId}: ${error.message}`));
      });
    }
    ...
    if (this.process.stdin) {
      this.process.stdin.on('error', (error) => {
        this.handleError(new Error(`Stdin error for agent ${this.agentId}: ${error.message}`));
      });
    }
    ...
    this.process.on('error', (error) => {
      this.handleError(new Error(`Process error for agent ${this.agentId}: ${error.message}`));
    });
  }
  ...
  private handleIncomingData(data: string): void {
    try {
      ...
      if (this.messageBuffer.length > this.options.maxMessageSize) {
        throw new Error(`Message buffer too large: ${this.messageBuffer.length} bytes exceeds limit`);
      }
      ...
      this.handleError(new Error(`Error processing incoming data for agent ${this.agentId}: ${error}`));
    }
  }
  ...
  private processBufferedMessages(): void {
    ...
    while ((delimiterIndex = this.messageBuffer.indexOf(delimiter)) !== -1) {
      ...
      try {
        ...
        this.connectionState.bytesReceived += Buffer.byteLength(message, this.options.encoding);
        this.connectionState.lastMessageTime = new Date();
        ...
        this.handleError(new Error(`Error handling message for agent ${this.agentId}: ${error}`));
      }
    }
  }
