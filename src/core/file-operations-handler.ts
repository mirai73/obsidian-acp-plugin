...
export class FileOperationsHandlerImpl implements FileOperationsHandler {
  ...
  constructor(config: FileOperationsConfig) {
    ...
    this.allowedExtensions = new Set(config.allowedExtensions || ['.md', '.txt', '.json']);
  ...
  async readTextFile(filePath: string): Promise<{content: string, encoding?: string}> {
    ...
    try {
      ...
      if (!this.validatePath(normalizedPath)) {
        const error = errorHandler.createInvalidPathError(filePath, 'Invalid path format');
      ...
      if (!stats.isFile()) {
        const error = errorHandler.createInvalidPathError(filePath, 'Path is not a file');
        // logger.fileOp('read', filePath, false, new Error(error.message), this.sessionId, this.userId);
        throw new Error(error.message);
      }
      ...
      if (this.allowedExtensions.size > 0 && !this.allowedExtensions.has(ext)) {
        const error = errorHandler.createPermissionDeniedError('read', filePath);
        // logger.fileOp('read', filePath, false, new Error(error.message), this.sessionId, this.userId);
        throw new Error(error.message);
      }
      ...
      if (nodeError.code === 'EACCES') {
        const enhancedError = errorHandler.createPermissionDeniedError('read', filePath);
        // logger.fileOp('read', filePath, false, error as Error, this.sessionId, this.userId);
        throw new Error(enhancedError.message);
      }
  ...
  async writeTextFile(filePath: string, content: string, encoding?: string): Promise<void> {
    ...
    try {
      ...
      const ext = path.extname(absolutePath).toLowerCase();
      if (this.allowedExtensions.size > 0 && !this.allowedExtensions.has(ext)) {
        const error = errorHandler.createPermissionDeniedError('write', filePath);
        // logger.fileOp('write', filePath, false, new Error(error.message), this.sessionId, this.userId);
        throw new Error(error.message);
      }

      // Check content size
      const contentSize = Buffer.byteLength(content, encoding as BufferEncoding || 'utf8');
      ...
      // Write file content
      await fs.writeFile(absolutePath, content, encoding as BufferEncoding || 'utf8');
      ...
      if (nodeError.code === 'EACCES') {
        const enhancedError = errorHandler.createPermissionDeniedError('write', filePath);
        // logger.fileOp('write', filePath, false, error as Error, this.sessionId, this.userId);
        throw new Error(enhancedError.message);
      }
