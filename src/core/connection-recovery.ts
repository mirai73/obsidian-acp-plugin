...
export class ConnectionRecovery {
	...
	async connect(): Promise<void> {
		if (
			this.status.state === ConnectionState.CONNECTING ||
	...
	private async attemptConnection(): Promise<void> {
		...
		const timeoutPromise = new Promise<never>((_, reject) => {
			this.connectionTimeout = setTimeout(() => {
				reject(
					errorHandler.createTimeoutError(
						'Connection attempt',
		...
		try {
			await Promise.race([this.connectFunction(), timeoutPromise]);
		} finally {
			this.clearConnectionTimeout();
		}
	}
	...
	private calculateBackoffDelay(): number {
		...
		const exponentialDelay = Math.min(
			this.config.initialDelay *
				Math.pow(this.config.backoffMultiplier, this.status.retryCount - 1),
			this.config.maxDelay
		);

		// Add jitter to prevent thundering herd
		const jitter =
			exponentialDelay * this.config.jitterFactor * (Math.random() - 0.5);
