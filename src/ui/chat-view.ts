...
export class ChatView extends ItemView implements ChatInterface {
	...
	private async handleSendMessage(): Promise<void> {
		...
		const isBuiltIn = builtInCommands.some((cmd) =>
	...
	private updateInputState(): void {
		...
		if (this.agentSelector) {
			...
			console.log(
				this.agentSelector.classList.toggle('is-readonly', hasMessages)
			);
		}
	...
	private updateAgentSelector(): void {
		...
		if (connectedAgentIds.length === 0) {
			this.agentSelector.createEl('option', {
	...
	private showSessionHistory(): void {
		...
		const modal = new SessionSuggestModal(this.app, sessions, (sessionId) =>
