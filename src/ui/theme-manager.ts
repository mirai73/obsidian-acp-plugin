...
export class ThemeManager extends Component {
  ...
  constructor(app: App, config: ThemeConfig = { mode: 'auto', respectSystemPreference: true }) {
    super();
    this.app = app;
    this.config = config;
  ...
  private getBaseThemeVariables(): Record<string, string> {
    ...
    return {
      ...
      '--acp-text-normal': getProp('--text-normal', isDark ? '#dcddde' : '#2e3338'),
      '--acp-text-muted': getProp('--text-muted', isDark ? '#888888' : '#72767d'),
      '--acp-text-faint': getProp('--text-faint', isDark ? '#4f545c' : '#b9bbbe'),
      '--acp-border-color': getProp('--background-modifier-border', isDark ? '#303030' : '#e0e0e0'),
