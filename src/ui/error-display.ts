...
export class ErrorDisplay {
  ...
  createErrorMessage(errorInfo: ErrorInfo): HTMLElement {
    ...
    if (this.config.enableRetryActions && this.shouldShowRetryAction(errorInfo)) {
  ...
  createConnectionStatus(
    ...
    if (isConnected) {
      ...
      text.textContent = retryCount && retryCount > 0 
  ...
  createErrorSummary(errors: ErrorInfo[]): HTMLElement {
    ...
    Object.entries(errorsBySeverity).forEach(([severity, severityErrors]) => {
      ...
      severityErrors.slice(0, 5).forEach(error => { // Show max 5 per severity
  ...
  private getSeverityIcon(severity: ErrorSeverity): string {
    switch (severity) {
      case ErrorSeverity.LOW: return 'info-icon';
      case ErrorSeverity.MEDIUM: return 'warning-icon';
      case ErrorSeverity.HIGH: return 'error-icon';
      case ErrorSeverity.CRITICAL: return 'critical-icon';
  ...
  private createContextDisplay(context: Record<string, any>): HTMLElement {
    ...
    const entries = Object.entries(context).slice(0, this.config.maxContextItems);
  ...
  private shouldShowRetryAction(errorInfo: ErrorInfo): boolean {
    ...
    return errorInfo.message.includes('connection') || 
  ...
  private groupErrorsBySeverity(errors: ErrorInfo[]): Record<string, ErrorInfo[]> {
