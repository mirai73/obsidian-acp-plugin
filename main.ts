...
export default class ACPChatPlugin extends Plugin {
  ...
  async onload() {
    ...
    const permissionManager = new PermissionManagerImpl(
      {
        ...
        requireConfirmation:
          this.settings.permissions?.showPermissionDialog !== false,
    ...
    this.app.workspace.onLayoutReady(async () => {
      ...
      if (defaultAgentId && this.chatView) {
        this.chatView.initSession(defaultAgentId).catch((err) =>
          console.error('Failed to init default session:', err)
        );
      }
    });
  }
  ...
  private normalizeDefaultAgent(): void {
    const agents = this.settings.agents;
    if (agents.length === 0) return;
    if (!agents.some((a) => a.id === this.settings.defaultAgentId)) {
      this.settings.defaultAgentId = agents[0].id;
    }
  }
...
class ACPChatSettingTab extends PluginSettingTab {
  ...
  private displayPermissionConfiguration(containerEl: HTMLElement): void {
    ...
    presets.forEach((preset) => {
      ...
      presetItem.createEl('p', {
        text: preset.description,
        cls: 'preset-description',
      });

      const applyButton = presetItem.createEl('button', {
        text: 'Apply Preset',
      });
      applyButton.addEventListener('click', async () => {
        ...
        this.plugin.settings.permissions.allowedPaths =
          preset.config.allowedPaths;
        this.plugin.settings.permissions.readOnlyPaths =
          preset.config.readOnlyPaths;
        this.plugin.settings.permissions.deniedPaths =
          preset.config.deniedPaths;
  ...
  private displayConnectionManagement(containerEl: HTMLElement): void {
    ...
    new Setting(diagnosticsContainer)
      ...
      .addButton((button) =>
        button.setButtonText('Run Test').onClick(() => {
  ...
  private displayUIConfiguration(containerEl: HTMLElement): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'Follow system dark/light mode preference when theme is set to Auto'
      )
      .addToggle((toggle) =>
        toggle
    ...
    const themeStatusContainer = containerEl.createDiv(
    ...
    const currentTheme =
      this.plugin.themeManager?.getCurrentTheme() || 'unknown';
    ...
    themeStatusContainer.createEl('p', {
      text: `Current Theme: ${currentTheme}`,
    });
    themeStatusContainer.createEl('p', {
      text: `System Preference: ${systemTheme}`,
    });
    themeStatusContainer.createEl('p', {
      text: `Obsidian Theme: ${document.body.classList.contains('theme-dark') ? 'dark' : 'light'}`,
    });
    ...
    new Setting(containerEl)
      ...
      .addButton((button) =>
        button.setButtonText('Toggle Theme').onClick(() => {
          this.plugin.toggleTheme();
          this.display(); // Refresh settings display
        })
      )
      .addButton((button) =>
        button.setButtonText('Refresh Theme').onClick(() => {
  ...
  private async connectAllAgents(): Promise<void> {
    const enabledAgents = this.plugin.settings.agents.filter(
      (agent) => agent.enabled
    );
  ...
  private runConnectionDiagnostics(container: HTMLElement): void {
    ...
    setTimeout(() => {
      ...
      const resultText = diagnosticsResult.createEl('pre', {
        cls: 'diagnostics-output',
      });
  ...
  private displayConnectionLogs(container: HTMLElement): void {
    ...
    if (!stats) {
      container.createEl('p', {
        text: 'No connection statistics available',
        cls: 'setting-item-description',
      });
      return;
    }
    ...
    statsList.createEl('li', {
      text: `Connected Agents: ${stats.connectedAgents}`,
    });
    ...
    if (stats.processStats.size > 0) {
      ...
      for (const [agentId, health] of stats.processStats) {
        processStatsList.createEl('li', {
          text: `${agentId}: ${health.status} (errors: ${health.errorCount}, restarts: ${health.restartCount})`,
        });
      }
    }
    ...
    if (stats.transportStats.size > 0) {
      ...
      const transportStatsList = transportStatsEl.createEl('div', {
        cls: 'activity-list',
      });
      for (const [agentId, transportStat] of stats.transportStats) {
        ...
        statItem.createEl('span', {
          text: `Messages: ${transportStat.jsonRpcStats?.messagesSent || 0} sent, ${transportStat.jsonRpcStats?.messagesReceived || 0} received`,
...
class AgentConfigModal extends Modal {
  ...
  onOpen() {
    ...
    nameContainer.createEl('div', {
      text: 'Display name for this agent',
      cls: 'setting-item-description',
    });
    ...
    commandContainer.createEl('div', {
      text: 'Command',
      cls: 'setting-item-name',
    });
    commandContainer.createEl('div', {
      text: 'Executable command to start the agent',
      cls: 'setting-item-description',
    });
    this.commandInput = commandContainer.createEl('input', {
      type: 'text',
      placeholder: 'python',
    });
    ...
    argsContainer.createEl('div', {
      text: 'Arguments',
      cls: 'setting-item-name',
    });
    argsContainer.createEl('div', {
      text: 'Command line arguments (space-separated)',
      cls: 'setting-item-description',
    });
    this.argsInput = argsContainer.createEl('input', {
      type: 'text',
      placeholder: '-m my_agent --acp',
    });
    this.argsInput.value = this.agent?.args?.join(' ') || '';

    // Working Directory
    const workingDirContainer = contentEl.createDiv('setting-item');
    workingDirContainer.createEl('div', {
      text: 'Working Directory',
      cls: 'setting-item-name',
    });
    workingDirContainer.createEl('div', {
      text: 'Working directory for the agent process (optional)',
      cls: 'setting-item-description',
    });
    this.workingDirInput = workingDirContainer.createEl('input', {
      type: 'text',
    ...
    this.environmentInput = envContainer.createEl('textarea', {
